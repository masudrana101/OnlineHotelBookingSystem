                    ...
                    <section class="panel">
                        ...
                        <div class="panel-body">
                            @if(session()->has('status'))
                                {!! session()->get('status') !!}
                            @endif
                            <form action="{{ route('manager.hotel.room.add', $hotel->id) }}" method="post">
                                @csrf
                                <div class="row">
                                    <div class="col-sm-6">
                                        ...
                                        <div class="form-group">
                                            ...
                                            <select name="room_type_id" required
                                                ...
                                                @foreach($roomTypes as $val)
                                                    <option value="{{ $val->id }}" @if(old('room_type_id') == $val->id) selected @endif>{{ $val->name.' ('.$val->capacity.' P)' }}</option>
                                                @endforeach
                                            </select>
                                            @error('room_type_id')
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-sm-6 mt-4">
                                        ...
                                        <label for="unavailable"><input type="radio" id="unavailable" class="mr-2"
                                                                     name="status" value="unavailable" @if(old('status') == 'unavailable') checked @endif> Unavailable</label>
                                    </div>
                                </div>
                                <div class="row">
                                    <div class="col-sm-12">
